    ...
    <div class="col-xl-7 col-xxl-7 mt-3 mb-3 pl-0">
        <div class="card flex-fill w-100">
            <div class="card-body">
                ...
                <div class="progress">
                    <div class="progress-bar" role="progressbar" style="width: {{ $approvalRate }}%;" aria-valuenow="{{ $approvalRate }}" aria-valuemin="0" aria-valuemax="100">{{ round($approvalRate, 2) }}%</div>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="card {{ $color }} ml-3 mb-5 " style="width: 18rem;">
                    <div class="card-body">
                        ...
                        @if (auth()->user()->id_level == 1 || auth()->user()->id_level == 2 || auth()->user()->id_level == 3 || auth()->user()->id_level == 4)
                            <a href="/posts"><p class="card-text text-white">Lihat Detail <i class="fa fa-angle-double-right ml-2"></i></p></a>
                        @endif
                    </div>
                </div>
            @endforeach
